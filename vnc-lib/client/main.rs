...
fn mask_cursor(
    ...
    fn read_color<R: Read>(reader: &mut R, size: usize, masks: &PixelMasks) -> IoResult<Color> {
        ...
        Ok(Color {
            r: ((packed as u32 & masks.rmask) >> masks.rmask.trailing_zeros()) as u8,
            g: ((packed as u32 & masks.gmask) >> masks.gmask.trailing_zeros()) as u8,
            b: ((packed as u32 & masks.bmask) >> masks.bmask.trailing_zeros()) as u8,
            a: 255,
        })
    }
    ...
    fn write_color<W: Write>(
        ...
        let packed = {
            ...
            ((r << masks.rmask.trailing_zeros()) & masks.rmask)
                | ((g << masks.gmask.trailing_zeros()) & masks.gmask)
                | ((b << masks.bmask.trailing_zeros()) & masks.bmask)
                | ((a << masks.amask.trailing_zeros()) & masks.amask)
        };
        writer
...
fn main() {
    ...
    let port = *matches.get_one::<u16>("PORT").unwrap_or(&5900);
    ...
    'running: loop {
                ...
                Event::PutPixels(vnc_rect, ref pixels) => {
                    ...
                    screen
                        .update(
                            Some(sdl_rect),
                            pixels,
                            sdl_rect.width() as usize * sdl_format.byte_size_per_pixel(),
                        )
                        .unwrap();
                    canvas
                ...
                Event::CopyPixels {
                    ...
                    screen
                        .update(
                            Some(sdl_dst),
                            &pixels,
                            sdl_dst.width() as usize * sdl_format.byte_size_per_pixel(),
                        )
                        .unwrap();
                    ...
                    if width > 0 && height > 0 {
                        ...
                        new_cursor
                            .update(
                                None,
                                &cursor_pixels,
                                width as usize * sdl_cursor_format.byte_size_per_pixel(),
                            )
                            .unwrap();
        ...
        match cursor {
            Some(ref cursor) => {
                ...
                let clipped_cursor_rect = raw_cursor_rect.intersection(screen_rect);
