                        ...
                        match rectangle.encoding {
                            ...
                            protocol::Encoding::Cursor => {
                                ...
                                let mut mask_bits = vec![
                                    0;
                                    (rectangle.width as usize).div_ceil(8)
                                        * (rectangle.height as usize)
                                ];
...
impl Client {
        ...
        Auth: FnOnce(&[AuthMethod]) -> Option<AuthChoice>,
    {
        ...
        version.write_to(&mut stream)?;

        let security_types = match version {
            protocol::Version::Rfb33 => {
                let security_type_raw = stream.read_u32::<BigEndian>()?;
                let security_type = match security_type_raw {
                    0 => protocol::SecurityType::Invalid,
                    1 => protocol::SecurityType::None,
                    2 => protocol::SecurityType::VncAuthentication,
                    30 => protocol::SecurityType::AppleRemoteDesktop,
                    n => protocol::SecurityType::Unknown(n as u8),
                };
                info!(
                    "<- SecurityType (3.3): {:?} ({})",
            ...
            _ => {
                ...
                info!("<- SecurityTypes: {:?}", security_types);
                security_types.0
            }
        };
        ...
        match version {
            ...
            _ => {
                ...
                info!("-> Selecting SecurityType: {:?}", used_security_type);
                protocol::SecurityType::write_to(&used_security_type, &mut stream)?;
            }
        }

        info!("Auth choice picked: {:?}", auth_choice);
        match auth_choice {
            AuthChoice::Password(mut password) => {
                ...
                info!("Reading 16-byte VNC challenge...");
                let mut challenge = [0; 16];
                stream.read_exact(&mut challenge)?;
                info!("Challenge received: {:02X?}", challenge);
                let response = des(&challenge, &password);
                info!("Sending 16-byte VNC response...");
        ...
        if !skip_security_result {
            info!("Waiting for SecurityResult...");
            match {
                let result = stream.read_u32::<BigEndian>()?;
                match result {
                    0 => Ok(protocol::SecurityResult::Succeeded),
                    1 => Ok(protocol::SecurityResult::Failed),
                    _ => {
                        log::error!("Unexpected security result value: 0x{:08X}", result);
                        Err(Error::Unexpected("security result"))
                    }
                }
            }? {
                protocol::SecurityResult::Succeeded => {
                    info!("SecurityResult: Succeeded");
                }
                protocol::SecurityResult::Failed => {
                    info!("SecurityResult: Failed");
                    match version {
